report 50107 "Sales Invoice ALHI"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Reports\SI - ALHI Pre-printed Stationary\Sales Invoice Fixed Lines.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            column(Amount_Excluding_VAT; Amount)
            {
            }
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(SalesPurchPerson; SalesPurchPerson."Phone No.")
                    {
                    }
                    column(ReferenceText; YourRfr_Lbl)
                    {
                    }
                    ...
                    column(QuanatityLbl; QuanatityLbl)
                    { }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(Item_Corss_Refernce_Text; ItemCorssRefernceTextG)
                        { }
                        ...
                        trigger OnAfterGetRecord()
                        begin
                            IF (CheckList.Count = 0) AND ("Sales Invoice Line".Type <> "Sales Invoice Line".Type::" ") THEN begin
                                SLNo += 1;
                                CheckList.Add(FORMAT("Sales Invoice Line".Type) + "Sales Invoice Line"."No." + "Sales Invoice Line".Description);
                            end;
                            if CheckList.Count <> 0 then
                                if not CheckList.Contains(FORMAT("Sales Invoice Line".Type) + "Sales Invoice Line"."No." + "Sales Invoice Line".Description) then
                            ...
                            // Start Ramesh
                            Clear(ItemCorssRefernceTextG);
                            if ItemG.Get("No.") then begin
                                ItemCrossReferenceG.SetRange("Item No.", ItemG."No.");
                                ItemCrossReferenceG.SetRange("Cross-Reference Type", ItemCrossReferenceG."Cross-Reference Type"::"Bar Code");
                                if ItemCrossReferenceG.FindSet() then
                                    repeat
                                        // ItemCorssRefernceTextG += ItemCrossReferenceG."Cross-Reference No." + ',';
                                        if ItemCorssRefernceTextG = '' then
                                            ItemCorssRefernceTextG := ItemCrossReferenceG."Cross-Reference No."
                                        else
                                            ItemCorssRefernceTextG := ItemCorssRefernceTextG + ',' + ItemCrossReferenceG."Cross-Reference No.";
                                    until ItemCrossReferenceG.Next() = 0;
                            end;
                            // ItemCorssRefernceTextG := CopyStr(ItemCorssRefernceTextG,1,StrLen(ItemCorssRefernceTextG)-1)
                        end;
            ...
            begin
                ...
                if SalesInvoiceLineL.FindSet() then
                    TotalLines := SalesInvoiceLineL.Count();
                ...
                TotalAmt := "Sales Invoice Header"."Amount Including VAT";
                tvar := (ROUND(TotalAmt) MOD 1 * 100);
                ConvertAmountInWord.InitTextVariable;
                ConvertAmountInWord.FormatNoText(AmtInwrdArray1, tvar, "Sales Invoice Header"."Currency Code");
                AmountInWords := AmtInwrdArray1[1];
                IF AmountInWords = '' THEN
                    AmountInWords := 'ZERO';
                ConvertAmountInWord.InitTextVariable;
                ConvertAmountInWord.FormatNoText(AmtInwrdArray2, TotalAmt, "Sales Invoice Header"."Currency Code");
                AmountInWords2 := AmtInwrdArray2[1];
                //AmountText1 := Text + ' ' + CurrCode + ' AND ' + AmtInwrdArray2 + ' ' + DecimalDec + ' ONLY';
                AmountText := "Sales Invoice Header"."Currency Code" + ' ' + AmountInWords2;//+ ' AND ' + AmountInWords + ' ONLY';
    ...
    begin
        ...
        OnAfterInitReport;
        FooterText1 := CompanyInfo.Name;
        IF CompanyInfo.Address <> '' THEN
            FooterText1 += ' * ' + CompanyInfo.Address;
        IF CompanyInfo."Address 2" <> '' THEN
            FooterText1 += ' * ' + CompanyInfo."Address 2";
        IF CompanyInfo."Phone No." <> '' THEN
            FooterText1 += ' * Tlf. ' + CompanyInfo."Phone No.";
        IF CompanyInfo."E-Mail" <> '' THEN
            FooterText1 += ' * Mail. ' + CompanyInfo."E-Mail";

        FooterText2 := 'Cvr. nr: ' + CompanyInfo."VAT Registration No.";
        IF CompanyInfo."Bank Name" <> '' THEN
            FooterText2 += ' * ' + CompanyInfo."Bank Name";
        IF CompanyInfo."Bank Branch No." <> '' THEN
            FooterText2 += ': ' + CompanyInfo."Bank Branch No.";
        IF CompanyInfo."Bank Account No." <> '' THEN
            FooterText2 += ' ' + CompanyInfo."Bank Account No.";
    end;
    ...
    var
        Text004: Label 'Sales Invoice', Comment = '%1 = Document No.';
        PageCaptionCap: Label 'Page %1 of %2';
        BarcodeLbl: Label 'Barcode';
        ItemG: Record Item;
        ItemCrossReferenceG: Record "Item Cross Reference";
        ...
        FooterText1: Text;
        FooterText2: Text;
        ItemCorssRefernceTextG: Text;
        OrderNoText: Text[80];
        SalesPersonText: Text[30];
        VATNoText: Text[80];
        ReferenceText: Text[80];
        YourRfr_Lbl: Label 'Your Reference';
        TotalLines: Integer;
        TotalText: Text[50];
        ...
        QuanatityLbl: label 'Qty';
        UnitPriceLbl: label 'Price Each';
        TypeLbl: label 'Type';
        TotalAmt: Decimal;
        tvar: Decimal;
